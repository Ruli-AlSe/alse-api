  ...
  describe 'Update profile action' do
    ...
    context 'when has been updated successfully' do
      before do
        ...
        put(:update, format: :json, params: { id: user_profile.id, profile: { name: 'New testing name', headliner: 'New testing headliner' } })
      end
      ...
      context 'response with correct profile structure' do
        ...
        it { is_expected.to include(:id, :name, :last_name, :headliner, :bio, :city, :state, :country, :phone_number, :social_media) }
      end
    end
    ...
    context 'when something was wrong' do
      before do
        request.headers.merge!(headers)
        put(:update, format: :json, params: { id: user_profile.id, profile: { title: 'New testing title', biography: 'New testing biography' } })
      end
